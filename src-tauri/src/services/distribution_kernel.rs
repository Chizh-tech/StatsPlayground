...
mod tests {
    ...
    #[test]
    fn quantile_box_public_letter_value_uses_weighted_type6_and_neff_depth() {
        let sample = group(
            (1..=16)
                .map(|value| observation(value, value as f64, 1.0, 1))
                .collect(),
        ...
        let kernel = quantile_box_public_letter_value(&sample).expect("quantile box");
        assert_eq!(kernel.status, QuantileBoxKernelStatusV1::Available);
        assert_eq!(kernel.reason_code, None);
        assert_eq!(kernel.layers.len(), 4);
        assert!((kernel.layers[0].probability_lower - 0.25).abs() <= 1e-12);
        assert!((kernel.layers[1].probability_lower - 0.125).abs() <= 1e-12);
        assert!(kernel.layers.iter().all(|layer| layer.lower <= layer.upper));
        assert!(
            (kernel.median - weighted_type6(&sample.observations, 0.5).expect("median")).abs()
                <= 1e-12
        );
    }

    #[test]
    fn quantile_box_public_letter_value_freq_matches_logical_expansion() {
        let compact = group(
            vec![
                observation(1, 1.0, 1.0, 3),
                observation(2, 4.0, 1.0, 2),
                observation(3, 9.0, 1.0, 1),
            ],
            0,
        );
        let expanded = group(
            vec![
                ...
                observation(3, 1.0, 1.0, 1),
                observation(4, 4.0, 1.0, 1),
                observation(5, 4.0, 1.0, 1),
                observation(6, 9.0, 1.0, 1),
            ],
            0,
        );

        let compact_kernel = quantile_box_public_letter_value(&compact).expect("compact");
        let expanded_kernel = quantile_box_public_letter_value(&expanded).expect("expanded");
        assert_eq!(compact_kernel.layers.len(), expanded_kernel.layers.len());
        for (left, right) in compact_kernel
            .layers
            .iter()
            .zip(expanded_kernel.layers.iter())
        {
            assert!((left.lower - right.lower).abs() <= 1e-12);
            assert!((left.upper - right.upper).abs() <= 1e-12);
            assert_eq!(left.depth, right.depth);
        }
    }

    #[test]
    fn stem_and_leaf_public_decimal_marks_weight_as_typed_unavailable() {
        let sample = group(
            vec![observation(1, 1.2, 2.0, 1), observation(2, 4.5, 1.0, 1)],
            0,
        );
        let kernel =
            stem_and_leaf_public_decimal(&sample, true, 200, 120).expect("stem unavailable");
    ...
    #[test]
    fn stem_and_leaf_public_decimal_selects_scale_from_span_and_caps_leaves_per_stem() {
        let sample = group(
            vec![
                observation(1, 0.0, 1.0, 1),
                observation(2, 204.0, 1.0, 1),
                observation(3, 11.0, 1.0, 1),
                observation(4, 12.3, 1.0, 130),
                observation(5, 19.0, 1.0, 1),
            ],
            0,
        );

        let kernel = stem_and_leaf_public_decimal(&sample, false, 200, 120).expect("stem rows");
        assert_eq!(kernel.status, StemAndLeafKernelStatusV1::Available);
        assert_eq!(kernel.reason_code, None);
        assert_eq!(kernel.scale, 10.0);
        let row_1 = kernel
            .rows
            .iter()
            .find(|row| row.stem == "1")
            .expect("stem 1");
        assert_eq!(row_1.leaves.len(), 120);
        assert_eq!(row_1.omitted_leaf_count, 12);
        assert_eq!(row_1.leaves[0], "1");
        assert_eq!(
            row_1
                .leaves
                .iter()
                .filter(|leaf| leaf.as_str() == "2")
                .count(),
            119
        );
    }
    ...
    #[test]
    fn stem_and_leaf_public_decimal_applies_max_display_stems_and_exact_omitted_counts() {
        let sample = group(
            vec![
                observation(1, 0.1, 1.0, 3),
                observation(2, 10.2, 1.0, 4),
                observation(3, 20.3, 1.0, 5),
            ],
            0,
        );

        let kernel = stem_and_leaf_public_decimal(&sample, false, 2, 3).expect("stem rows");
        assert_eq!(kernel.status, StemAndLeafKernelStatusV1::Available);
        assert_eq!(kernel.scale, 1.0);
        assert_eq!(kernel.rows.len(), 2);
        assert_eq!(kernel.omitted_stem_count, 1);
        assert_eq!(kernel.omitted_leaf_count, 6);
        ...
        let row_0 = kernel
            ...
            .find(|row| row.stem == "0")
            .expect("stem 0");
        assert_eq!(row_0.leaves, vec!["1", "1", "1"]);
        assert_eq!(row_0.omitted_leaf_count, 0);

        let row_10 = kernel
            .rows
            .iter()
            .find(|row| row.stem == "10")
            .expect("stem 10");
        assert_eq!(row_10.leaves, vec!["2", "2", "2"]);
        assert_eq!(row_10.omitted_leaf_count, 1);
    }

    #[test]
    fn stem_and_leaf_public_decimal_chooses_decimal_power_scale_for_negative_positive_span() {
        let sample = group(
            vec![observation(1, -55.0, 1.0, 1), observation(2, 45.0, 1.0, 1)],
            0,
        );
    ...
    #[test]
    fn stem_and_leaf_public_decimal_handles_negative_zero_decimal_and_frequency_ordering() {
        let sample = group(
            vec![
                observation(1, -12.3, 1.0, 1),
                observation(2, -11.9, 1.0, 1),
                observation(3, -0.0, 1.0, 1),
                observation(4, 0.0, 1.0, 1),
                observation(5, 10.8, 1.0, 1),
                observation(6, 10.2, 1.0, 2),
                observation(7, 10.5, 1.0, 1),
                observation(8, 20.0, 1.0, 1),
            ],
            0,
        );
        ...
        let row_neg_12 = kernel
            .rows
            .iter()
            .find(|row| row.stem == "-12")
            .expect("stem -12");
        assert_eq!(row_neg_12.leaves, vec!["3"]);

        let row_neg_11 = kernel
            .rows
            .iter()
            .find(|row| row.stem == "-11")
            .expect("stem -11");
        assert_eq!(row_neg_11.leaves, vec!["9"]);
        ...
        let row_zero = kernel
            ...
            .find(|row| row.stem == "0")
            .expect("stem 0");
        assert_eq!(row_zero.leaves, vec!["0", "0"]);
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum QuantileBoxKernelStatusV1 {
    Available,
    Unavailable,
    Failed,
}
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct QuantileBoxLayerDataV1 {
...
#[derive(Debug, Clone, PartialEq)]
pub(crate) struct QuantileBoxKernelV1 {
    pub layers: Vec<QuantileBoxLayerDataV1>,
    pub median: f64,
    pub status: QuantileBoxKernelStatusV1,
    pub reason_code: Option<String>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum StemAndLeafKernelStatusV1 {
    Available,
    Unavailable,
    Failed,
}
...
pub(crate) fn quantile_box_public_letter_value(
    sample: &PreparedGroupV1,
) -> Result<QuantileBoxKernelV1, AppError> {
    if sample.observations.is_empty() {
        return Ok(QuantileBoxKernelV1 {
            layers: Vec::new(),
            median: 0.0,
            status: QuantileBoxKernelStatusV1::Unavailable,
            reason_code: Some("distribution.method.emptyGroup".to_string()),
        });
    }
    let summary = continuous_summary(sample, 0.95)?;
    let n_eff = summary.n_eff.max(1.0);
    let max_depth = n_eff.log2().floor().max(1.0) as u64;
    let max_depth = max_depth.min(16);
    let mut layers = Vec::with_capacity(max_depth as usize);

    for depth in 1..=max_depth {
        let tail_probability = 0.5_f64.powi(depth as i32 + 1);
        let probability_lower = tail_probability;
        let probability_upper = 1.0 - tail_probability;
        if !(0.0..1.0).contains(&probability_lower)
            || !(0.0..1.0).contains(&probability_upper)
            || probability_lower >= probability_upper
        {
            break;
        }
        let lower = weighted_type6(&sample.observations, probability_lower)?;
        let upper = weighted_type6(&sample.observations, probability_upper)?;
        layers.push(QuantileBoxLayerDataV1 {
            probability_lower,
            probability_upper,
            lower,
            upper,
    ...
    let median = weighted_type6(&sample.observations, 0.5)?;
    Ok(QuantileBoxKernelV1 {
        layers,
        median,
        status: QuantileBoxKernelStatusV1::Available,
        reason_code: None,
    })
}

pub(crate) fn stem_and_leaf_public_decimal(
    sample: &PreparedGroupV1,
    has_weight_column: bool,
    max_display_stems: usize,
    max_leaves_per_stem: usize,
) -> Result<StemAndLeafKernelV1, AppError> {
    if max_display_stems == 0 || max_leaves_per_stem == 0 {
        return Err(AppError::InvalidParam(
            "distribution.config.stemLeafDisplayBudgetInvalid".to_string(),
        ));
    }
    if has_weight_column {
        return Ok(StemAndLeafKernelV1 {
            ...
            reason_code: Some("stemLeaf.weightUnsupported.v1".to_string()),
        });
    }
    if sample.observations.is_empty() {
        return Ok(StemAndLeafKernelV1 {
    ...
    for observation in &sorted {
        let normalized = if observation.y == 0.0 {
            0.0
        } else {
            observation.y
        } / scale;
        ...
        let remaining_capacity = max_leaves_per_stem.saturating_sub(entry.leaves.len()) as u64;
        let emitted = remaining_capacity.min(frequency);
        for _ in 0..emitted {
            entry.leaves.push(leaf.clone());
        }
        entry.omitted_leaf_count = entry
            .omitted_leaf_count
            .checked_add(frequency - emitted)
            .ok_or_else(|| {
                AppError::InvalidParam("distribution.method.frequencyOverflow".to_string())
            })?;
    }
    ...
    let displayed_stems = total_stems.min(max_display_stems);
    ...
    for (index, ((stem_value, signed_zero_order), accumulator)) in stems.into_iter().enumerate() {
        if index < max_display_stems {
            ...
            omitted_leaf_count = omitted_leaf_count
                .checked_add(accumulator.total_leaf_count)
                .ok_or_else(|| {
                    AppError::InvalidParam("distribution.method.frequencyOverflow".to_string())
                })?;
        }
...
fn stem_leaf_scale_from_span(span: f64) -> f64 {
    if !span.is_finite() || span <= 0.0 {
        return 1.0;
    }

    let mut best_scale = 1.0;
    let mut best_bucket = u8::MAX;
    let mut best_range_distance = f64::INFINITY;
    let mut best_center_distance = f64::INFINITY;
    let base_exponent = span.log10().floor() as i32;

    for exponent in (base_exponent - 24)..=(base_exponent + 24) {
        let scale = 10_f64.powi(exponent);
        if !scale.is_finite() || scale <= 0.0 {
            continue;
        }
        let stem_span = span / scale;
        if !stem_span.is_finite() || stem_span <= 0.0 {
            continue;
        }

        let in_target = (10.0..=50.0).contains(&stem_span);
        let bucket = if in_target { 0 } else { 1 };
        let range_distance = if stem_span < 10.0 {
            10.0 - stem_span
        } else if stem_span > 50.0 {
            stem_span - 50.0
        } else {
            0.0
        };
        let center_distance = (stem_span - 20.0).abs();

        let better = bucket < best_bucket
            || (bucket == best_bucket && range_distance < best_range_distance)
            || (bucket == best_bucket
