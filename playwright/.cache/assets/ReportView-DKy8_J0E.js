import { a as __toESM, i as __exportAll, n as require_react, r as __commonJSMin, t as useTranslation } from "./index-ahixs0Dz.js";
...
function name(name, options) {
	return ((options || emptyOptions$4).jsx ? nameReJsx : nameRe).test(name);
}
//#endregion
...
function createElementProps(state, node) {
	...
	/** @type {string} */
	let prop;
	for (prop in node.properties) if (prop !== "children" && own$6.call(node.properties, prop)) {
...
function transformStylesToCssCasing(domCasing) {
	...
	/** @type {string} */
	let from;
	for (from in domCasing) if (own$6.call(domCasing, from)) cssCasing[transformStyleToCssCasing(from)] = domCasing[from];
	return cssCasing;
}
/**
	...
	*/
	function compile(events) {
		...
		while (++index < events.length) {
			...
			if (own$5.call(handler, events[index][1].type)) handler[events[index][1].type].call(Object.assign({ sliceSerialize: events[index][2].sliceSerialize }, context), events[index][1]);
		}
...
function extension(combined, extension) {
	/** @type {keyof Extension} */
	let key;
	for (key in extension) if (own$5.call(extension, key)) switch (key) {
		case "canContainEols": {
...
function post(tree, options) {
	...
	for (const deprecation of deprecations) if (Object.hasOwn(options, deprecation.from)) "" + deprecation.from + (deprecation.to ? "use `" + deprecation.to + "` instead" : "remove it") + changelog + deprecation.id;
...
function exitCodeText(token) {
	...
	if (this.data.inTable) value = value.replace(/\\([\\|])/g, replace$1);
	const node = this.stack[this.stack.length - 1];
	node.type;
	node.value = value;
	this.exit(token);
...
new RegExp("^" + /<(\/?)(iframe|noembed|noframes|plaintext|script|style|title|textarea|xmp)(?=[\t\n\f\r />])/gi.source, "i");
//#endregion
...
//#endregion
//#region src/utils/reportParser.ts
var REPORT_EMBED_RE = /^\{\{sp-embed kind="(table|graph|fitYByX|tabulate)" id="([^"{}\s\x00-\x1f\x7f]+)"\}\}$/;
...
function splitLineSegments(text) {
	...
	while (index < text.length) {
		...
		segments.push({
			content: text.slice(index, newlineIndex),
			newline
		});
...
function flushMarkdown(tokens, buffer) {
	if (!buffer) return "";
	const last = tokens[tokens.length - 1];
	if (last?.type === "markdown") last.markdown += buffer;
	else tokens.push({
		type: "markdown",
		markdown: buffer
	});
...
function formatReportEmbed(dependency) {
...
function parseReportMarkdown(markdown) {
	...
	for (const segment of splitLineSegments(markdown)) {
		...
		} else fence = isFenceLine(line);
	}
	flushMarkdown(tokens, buffer);
	return tokens;
}
//#endregion
//#region src/components/report/ReportMarkdown.tsx
var EMBED_ICON_CLASS = {
...
function leadingLineBreakLength(text) {
	if (text.startsWith("\r\n")) return 2;
	if (text.startsWith("\n") || text.startsWith("\r")) return 1;
	return 0;
}
function insertReportEmbed(markdown, selectionStart, selectionEnd, dependency) {
...
function ReportMarkdown({ markdown, tableOptions, graphOptions, fitYByXOptions, tabulateOptions }) {
	const { t } = useTranslation();
	const optionMaps = (0, import_react.useMemo)(() => ({
	...
	const tokens = (0, import_react.useMemo)(() => parseReportMarkdown(markdown), [markdown]);
	if (markdown.trim().length === 0) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
		className: "sp-report-preview-empty",
		children: t("report.previewEmpty", { defaultValue: "Preview updates as you type." })
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "sp-report-markdown-flow",
		children: tokens.map((token, index) => {
			if (token.type === "markdown") return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Markdown, {
				remarkPlugins: [remarkGfm],
				skipHtml: true,
				children: token.markdown
			}, `markdown:${index}`);
			const documentName = optionMaps[token.dependency.kind].get(token.dependency.documentId);
			const kindLabel = t(`report.group.${token.dependency.kind}`, { defaultValue: token.dependency.kind === "fitYByX" ? "Fit Y by X" : token.dependency.kind === "tabulate" ? "Tabulate" : token.dependency.kind === "graph" ? "Graphs" : "Tables" });
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "sp-report-embed-placeholder",
				"data-kind": token.dependency.kind,
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", {
					className: EMBED_ICON_CLASS[token.dependency.kind],
					"aria-hidden": "true"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("report.embedPlaceholder", {
...
//#endregion
//#region src/components/report/ReportView.tsx
var NARROW_VIEW_QUERY = "(max-width: 900px)";
function ReportView({ item, tableOptions, graphOptions, fitYByXOptions, tabulateOptions, onMarkdownChange, readOnly = false }) {
	const { t } = useTranslation();
	const textareaRef = (0, import_react.useRef)(null);
	const menuRef = (0, import_react.useRef)(null);
	const pendingSelectionRef = (0, import_react.useRef)(null);
	const [isInsertMenuOpen, setIsInsertMenuOpen] = (0, import_react.useState)(false);
	const [isNarrow, setIsNarrow] = (0, import_react.useState)(() => {
		if (typeof window === "undefined" || typeof window.matchMedia !== "function") return false;
		return window.matchMedia(NARROW_VIEW_QUERY).matches;
	});
	const [mobilePane, setMobilePane] = (0, import_react.useState)("editor");
	const groupedOptions = (0, import_react.useMemo)(() => [
		{
			kind: "table",
			label: t("report.group.table", { defaultValue: "Tables" }),
			items: tableOptions
		},
		{
			kind: "graph",
			label: t("report.group.graph", { defaultValue: "Graphs" }),
			items: graphOptions
		},
		{
			kind: "fitYByX",
			label: t("report.group.fitYByX", { defaultValue: "Fit Y by X" }),
			items: fitYByXOptions
		},
		{
			kind: "tabulate",
			label: t("report.group.tabulate", { defaultValue: "Tabulate" }),
			items: tabulateOptions
		}
	...
	const handleInsert = (kind, documentId) => {
		const textarea = textareaRef.current;
		if (!textarea || readOnly) return;
		const result = insertReportEmbed(item.markdown, textarea.selectionStart, textarea.selectionEnd, {
			kind,
			documentId
		});
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "sp-report-view",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "sp-panel-header sp-report-titlebar",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "sp-panel-header-title",
				children: item.name
			}), isNarrow ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "sp-report-mode-toggle",
				role: "tablist",
				"aria-label": t("report.modeLabel", { defaultValue: "Report view mode" }),
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					type: "button",
					className: `sp-report-mode-btn${mobilePane === "editor" ? " is-active" : ""}`,
					onClick: () => setMobilePane("editor"),
					children: t("report.editor", { defaultValue: "Editor" })
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
					type: "button",
					className: `sp-report-mode-btn${mobilePane === "preview" ? " is-active" : ""}`,
					onClick: () => setMobilePane("preview"),
					children: t("report.preview", { defaultValue: "Preview" })
				})]
			}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "sp-report-title-hint",
				children: t("report.titleHint", { defaultValue: "Markdown editor with live preview" })
			})]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "sp-report-panes",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", {
								...
								"aria-hidden": "true"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("report.insert", { defaultValue: "Insert" }) })]
						}), isInsertMenuOpen && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "sp-report-insert-menu",
							role: "menu",
							children: groupedOptions.map((group) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "sp-report-insert-group",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "sp-report-insert-group-label",
									children: group.label
								}), group.items.map((option) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
									type: "button",
									role: "menuitem",
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					className: "sp-report-preview",
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ReportMarkdown, {
...
//#endregion
export { ReportView, require_jsx_runtime as t };

//# sourceMappingURL=ReportView-DKy8_J0E.js.map